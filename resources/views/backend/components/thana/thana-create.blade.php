...
<script>
    ...
    async function UpazilaList() {
        try {
            ...
            allUpazila = response.data.UpazilasData; // Store all Upazila for filtering
        } catch (error) {
    ...
    function filterUpazilasByDistrict(districtId) {
        const filteredUpazila = allUpazilas.filter((upazila) => upazila.district_id == districtId);
